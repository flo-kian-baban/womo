...
**[FACT] S3b — THIS TABLE IS THE QUEUE.** There is no in-memory job list. `creator.submit` writes a `pending` capture row durably and the worker (`server/queue/analysisQueue.ts`) drives everything from here. `extract_commit` now runs in the phase runner, so a campaign is **5 runner rows** rather than 4 plus a hand-written shadow row.

The worker needs **two** queries, and the distinction matters:
- **`scanReadyWork`** — READY WORK: `status IN ('pending','failed','blocked')`, backoff gate passed, and **failure_class not in ('structural','genuine_empty')** (the S1 contract's `isRequeueable` — without that clause a permanently-failed phase reads as ready on every tick and is retried forever).
- **`findIncompleteCampaigns`** — UNFINISHED CAMPAIGNS, within a **24-hour** window. A campaign whose collection phases are all `complete` with no `extract_commit` row has nothing pending, running, parked or failed: the ready-scan cannot see it, yet it is plainly unfinished. Readiness is not always representable as a row status.

**[GOTCHA] Phase `output` carries NO schema version.** The 24-hour resumption bound is not arbitrary: a pre-S2 campaign was once resumed whose `augment` output still had the S1 shadow-bank shape (`{searchTitles: []}`) instead of the phase shape (`{pool: {...}}`), and the assembly died on `Cannot read properties of undefined (reading 'videoTitles')`. **Widening that window means replaying old outputs against current code.** Add a version to `output` before you do.

**[FACT] S3b — stale `running` reclaim.** A crash leaves rows stuck in `running`, and `running` is invisible to the scan, so those campaigns would be lost. `heartbeatPhase` touches `updated_at` every 30s while a phase holds a permit; `reclaimStaleRunning` returns rows quiet for more than **10 minutes** (5x transcribe's 120s budget) to `pending`. **Do NOT boot-reclaim indiscriminately** — this database is shared across analysts running locally, so "my process just started" says nothing about whose campaign a row belongs to. Only silence does.

**[FACT] S3b — writes split by purpose.** `recordPhaseState` **throws**; `recordPhaseObservation` is the best-effort wrapper. Queue state (enqueue, terminal outcome) must be durable, because a swallowed enqueue is a campaign nobody will run and nobody can see. Observation (banked output, pending/running markers) stays best-effort. The asymmetry is the point: losing a banked *output* costs one phase, losing the *enqueue* row costs the campaign.

**[FACT] S3b — a campaign that cannot complete records a terminal row.** Otherwise it is "unfinished" forever: re-offered on every drain and shown to the analyst as queued with no reason. A min-data refusal records `genuine_empty` (a deliberate, terminal "we will not extract from this"); anything else records `structural`, parked for a human.
...
| 20260729202441 | **womo_0013_run_inputs** (brand audit: new table `run_inputs`, one row per analysis run, holding what the OPERATOR submitted — `submitted_subject` NOT NULL plus nullable `google_maps_url` / `instagram_handle` / `tiktok_channel_url` / `brand_name`. **Additive only.** `run_id` is the PK and deliberately NOT an FK, same convention as womo_0009. Fixes a silent data loss: locators travelled only inside `analysis_phase_state.subject_hint`, a **varchar(160) identity key**, and six brand runs truncated at exactly 160 chars — `decodeSubject`'s JSON.parse then failed and its catch dropped ALL extras silently, so persistence recorded "no Instagram handle provided" for handles that had been supplied. `text` throughout: a length ceiling on operator data is the defect. **NULL means the operator supplied none, not that capture failed** — which is what lets "this brand has no TikTok" be a fact rather than a failure) |
...
> *Note:* the original work order referenced "7 migrations"; there are now **17** — `womo_0004`..`womo_0013` were applied in later sessions. `womo_0008` is a data correction (no schema change). This doc reflects the live state.
