...
| **S5 removal** — the pre-queue monolith deleted | `researchCreator` and the three functions reachable only through it (`researchTikTokCreator`, `researchInstagramCreator`, `researchYouTubeCreator`) plus `researchBrand`, removed once every caller was gone. The live path is the queue → campaign → phases. Provenance comments that cite them keep their attribution and are marked *(removed in S5)*; the audit body below is unchanged and still describes the code as it was audited | this session |
